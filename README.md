...
- A(w) — |w|~a~  
- B(w) — |w|~b~  
- C(w) — сумма трёх мер: |w|~aa~ + |w|~bb~ + суммарное расстояние от каждой b до конца слова по индексу b
...
Так как первый и последний символы l при применении правила не изменяются, а в мерах |w|~aa~ и |w|~bb~ необходимо смотреть только на пары подряд идущих символов, то вне зависимости от контекста |w|~aa + |w|~bb уменьшается на 2.  
...
1. |w|~aa~ — убывает либо не изменяется  
2. |w|~b~ mod 2 — не изменяется
